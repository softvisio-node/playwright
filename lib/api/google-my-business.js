...
export default class GoogleMyBusiness extends Events {
    #browser;
    #ctxOptions;
    #page;
    #maxRetries;

    constructor ( browser, ctxOptions, { maxRetries = 10 } = {} ) {
        super();
    ...
    // public
    async search ( { keyword, businessName, maxResults = 100, language, emulateUser } ) {
        var res;

        try {

            // open gmb
            res = await this.#repeatAction( this.#openGmb.bind( this ), language );
            if ( !res.ok ) throw res;
