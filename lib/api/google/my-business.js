...
export default class GoogleMyBusiness {
    ...
    constructor ( { browser, datasets, maxRetries = 10 } = {} ) {
    ...
    // public
    async search ( { keyword, businessName, maxResults = 100, location, radius, proxy, language, emulateUser } ) {
        var res;

        try {

            // open gmb
            res = await this.#repeatAction( this.#openGmb.bind( this ), { location, radius, proxy, language } );
            if ( !res.ok ) throw res;
    ...
    // private
    // XXX throw
    async #createPage ( { location, radius, proxy } ) {
        var coordinates;

        if ( typeof location === "string" ) {
            const geolocation = await this.#datasets.call( "geotargets/get-geotarget", location, { "random_coordinates": true } );

            if ( !geolocation.data?.random_coordinates ) throw result( [500, `Unable to get coordinates`] );

            coordinates = geolocation.data.random_coordinates;
        }
        else {
            if ( radius ) {
                coordinates = randomLocation.randomAnnulusPoint( location, 1, radius );
            }
            else {
        ...
        // proxy
        if ( proxy ) {
            options.proxy = await ProxyClient.new( proxy ).getPlaywrightProxy();
        }
    ...
    async #openGmb ( { location, radius, proxy, language } ) {
        this.#page = await this.#createPage( { location, radius, proxy } );
