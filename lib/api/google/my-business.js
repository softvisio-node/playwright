...
export default class GoogleMyBusiness {
    ...
    async search ( { keyword, businessName, maxResults = 100, location, minDistance, maxDistance, proxy, language, emulateUser } ) {
        ...
        for ( let n = 0; n < this.#maxRetries; n++ ) {
            try {
                ...
                // open gmb
                res = result.try( await this.#openGmb( { location, minDistance, maxDistance, proxy, language, emulateUser } ), { "allowUndefined": true } );
                if ( !res.ok ) break;

                // search keyword
                res = result.try( await this.#search( { keyword, emulateUser } ), { "allowUndefined": true } );
                if ( !res.ok ) break;
            }
            catch ( e ) {
                res = result.catch( e );
        ...
        if ( res.ok ) {
            try {
                ...
                // get results
                res = result.try( await this.#getResults( maxResults, businessName, emulateUser ) );
            }
            catch ( e ) {
                res = result.catch( e );
            }
    ...
    async #openGmb ( { location, minDistance, maxDistance, proxy, language, emulateUser } ) {
        this.#cleanup();
        ...
        try {
            ...
            if ( !emulateUser ) await this.#page.disableImages();
        }
        catch ( e ) {
            res = result.catch( e );
        }
    ...
    async #search ( { keyword, emulateUser } ) {
        ...
        var [res] = await Promise.all( [
            ...
            el.type( keyword + "\n", { "delay": emulateUser ? 200 : undefined } ),
        ] );
