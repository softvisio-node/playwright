    ...
    async search ( { keyword, businessName, maxResults = 100, location, minDistance, maxDistance, proxy, language, emulateUser } ) {
        ...
        for ( let n = 0; n < this.#maxRetries; n++ ) {
            try {
                ...
                // open gmb
                res = result.try( await this.#openGmb.bind( { location, minDistance, maxDistance, proxy, language } ), { "allowUndefined": true } );
                if ( !res.ok ) break;

                // search keyword
                res = result.try( await this.#search.bind( keyword, emulateUser ), { "allowUndefined": true } );
                if ( !res.ok ) break;
            }
            catch ( e ) {
                res = result.catch( e );
        ...
        if ( res.ok ) {
            try {
                ...
                // get results
                res = result.try( await this.#getResults.bind( maxResults, businessName, emulateUser ) );
            }
            catch ( e ) {
                res = result.catch( e );
            }
