...
const STATIC_DATA = fs.config.read( "./staticData.json", { "resolve": import.meta.url } );

export default {
    onPageCreated () {
        return withUtils().evaluateOnNewDocument( ( utils, { opts, STATIC_DATA } ) => {
            if ( !window.chrome ) {
                ...
                "apply": function ( target, ctx, args ) {
                    ...
                    const validateConnectInfo = ci => {
                        ...
                        Object.entries( ci ).forEach( ( [k, v] ) => {
                            ...
                            const MismatchError = ( propName, expected, found ) => TypeError( errorPreamble + `Error at property '${propName}': Invalid type: expected ${expected}, found ${found}.` );
